        ...
        <div class="bg-white/90 backdrop-blur rounded-3xl shadow-xl border border-white p-8 sm:p-10">
            ...
            <form class="mt-8 space-y-5" action="{{ route('register.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="space-y-1.5"><label class="text-xs font-bold text-slate-700 uppercase tracking-wider block">Nama Lengkap</label><input name="name" value="{{ old('name') }}" required class="block w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 bg-slate-50/50" placeholder="Nama lengkap"></div>
