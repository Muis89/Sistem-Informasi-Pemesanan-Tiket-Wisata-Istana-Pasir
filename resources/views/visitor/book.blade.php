                        ...
                        @forelse($tikets as $index => $tiket)
                            <label class="ticket-label relative flex items-center justify-between p-4 bg-white border-2 border-slate-200 rounded-2xl cursor-pointer hover:bg-slate-50 transition-all group">
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" name="tikets[{{ $index }}][id]" value="{{ $tiket->id }}" data-price="{{ $tiket->harga }}" data-name="{{ $tiket->nama_tiket }}" class="h-4.5 w-4.5 text-amber-600 focus:ring-amber-500 border-slate-300" onchange="toggleQty(this, '{{ $index }}')">
                                    <input type="hidden" name="tikets[{{ $index }}][quantity]" value="0" id="qty-{{ $index }}" disabled>
                                    <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="font-extrabold text-sm text-slate-900">Rp {{ number_format($tiket->harga, 0, ',', '.') }}</span>
                                    <input type="number" min="1" max="{{ $tiket->stok }}" value="1" id="input-qty-{{ $index }}" class="hidden w-16 px-2 py-1 border border-slate-200 rounded-lg text-sm text-center" oninput="updateHiddenQty(this, '{{ $index }}')" onclick="event.stopPropagation()">
                                </div>
                            </label>
                        @empty
        ...
        <div class="lg:col-span-5 space-y-6">
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm sticky top-24 space-y-6">
                ...
                <div class="space-y-4 text-sm font-medium">
                    <div class="flex justify-between text-slate-500"><span>Jenis Tiket</span><div id="summary-ticket" class="text-slate-900 font-bold text-right">-</div></div>
...
function toggleQty(checkbox, index){const input=document.getElementById('input-qty-'+index);const hidden=document.getElementById('qty-'+index);if(checkbox.checked){input.classList.remove('hidden');input.disabled=false;hidden.disabled=false;hidden.value=input.value}else{input.classList.add('hidden');input.disabled=true;hidden.disabled=true;hidden.value=0}calculateTotal()}
function updateHiddenQty(input, index){document.getElementById('qty-'+index).value=input.value;calculateTotal()}
function calculateTotal(){let total=0,items=[];document.querySelectorAll('.ticket-label input[type=checkbox]:checked').forEach(checkbox=>{const index=checkbox.name.match(/\[(\d+)\]/)[1];const qty=parseInt(document.getElementById('qty-'+index).value);const price=parseInt(checkbox.dataset.price);total+=qty*price;items.push(checkbox.dataset.name+' ('+qty+'x)')});document.getElementById('summary-ticket').innerHTML=items.length>0?items.join('<br>') : '-';document.getElementById('summary-period').innerText=formatDate(document.getElementById('visit_start_date').value)+' - '+formatDate(document.getElementById('visit_end_date').value);document.getElementById('summary-total').innerText=rupiah(total)}
